...
/**
 * Déclare `alert_case.process` (P008-A). Faire progresser un dossier accepté vers l'état « en traitement » (CaseDispatchService::process).
 *
 * Clé adaptée en deux segments (`alert_case.{action}`) : la note P008-A suggère `alert.case.process`, mais `governance.capability_definitions_stable_key_format_check` exige exactement un point (`^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$`) — vérifié au sens de la mission P008-A §10 (« vérifier les conventions du catalogue Governance avant de les créer »), pas une réinterprétation silencieuse.
 *
 * Portée réelle : `organization_id`. Mise à jour opérationnelle déjà couverte par l'engagement pris à `accept`.
 *
...
return new class extends Migration
{
    public function up(): void
    {
        DB::table('governance.capability_definitions')->insert([
            'id' => (string) Str::uuid7(),
            'stable_key' => 'alert_case.process',
            'version' => 1,
            'domain' => 'alerts',
            'action' => 'process',
            'description' => "Faire progresser un dossier accepté vers l'état « en traitement » (CaseDispatchService::process).",
            'operation' => 'write',
            'risk_class' => 'ordinary',
    ...
    {
        DB::table('governance.capability_definitions')->where('stable_key', 'alert_case.process')->delete();
    }
};
