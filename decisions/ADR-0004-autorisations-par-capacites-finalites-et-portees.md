...
## Addendum 2026-07-30 — Auto-amorçage de l'Administrateur Système (décision du fondateur)

**État :** adopté par le fondateur le 2026-07-30, en conversation, le jour même de l'amendement ci-dessus.

**Motif.** Claude Code a signalé que la première attribution de `governance.system_administrator` restait soumise à la séparation des tâches classique (sujet, auteur et approbateur distincts) : en production, seuls deux comptes réels existaient (le fondateur et un second compte), rendant impossible de réunir trois personnes distinctes pour ce seul octroi initial. Le fondateur a explicitement rejeté ce modèle : il veut un compte unique, immédiatement opérationnel par lui-même dès sa création (créer des comptes, attribuer des rôles, approuver ou refuser des campagnes), sans dépendre d'un second ou troisième compte réel pour exister.

**Décision.** L'exception de l'amendement ci-dessus s'étend désormais à l'attribution de `governance.system_administrator` lui-même, pas seulement aux octrois qu'il émet une fois détenu. Un compte peut donc se proposer et s'activer lui-même ce rôle, seul, sans auteur ni approbateur distinct — mais uniquement lorsqu'aucun grant actif `governance.system_administrator` n'existe déjà (`MultipleSystemAdministratorsRefusedException`, inchangée, continue de garantir qu'un seul compte à la fois détient ce rôle ; toute tentative pendant qu'un titulaire actif existe reste refusée quels que soient les comptes en présence). Ce chemin se rouvre symétriquement après toute révocation du titulaire en place — l'auto-amorçage n'est jamais réservé à la toute première fois dans l'histoire du système, seulement à chaque fois qu'aucun titulaire actif n'existe.

**Portée exacte de ce qui change :** uniquement la séparation des tâches sur l'attribution de `governance.system_administrator` lui-même. Rien d'autre ne change : les invariants C0, l'absence d'écriture directe dans `ledger.*`, la journalisation intégrale et la limite du bris de glace (§ précédent) restent identiques et non affectés par ce chemin d'amorçage.

**Ce que cela signifie concrètement :** tant qu'aucun compte ne détient `governance.system_administrator`, quiconque exécute `governance:grant-staff-capability governance.system_administrator <email>` peut désormais se désigner lui-même comme sujet, auteur et approbateur du même octroi. C'est un changement de posture de sécurité assumé : la garantie qu'« aucun acteur n'est jamais l'unique contrôleur de sa propre habilitation » (§ précédent, art. 18 §9/§19) ne s'applique plus à ce cas précis. Le fondateur en a été informé explicitement avant adoption.